...
type FilterMode = 'all' | 'known' | 'unknown' | 'late';
type SortKey = 'rank' | 'name' | 'calls' | 'duration';
...
export default function CallSummary({
  ...
  const lateCount = dataRows.filter((r) => r.hasLateCall).length;
  const topCaller = dataRows[0];
  const hasResults = summary.length > 0;

  const filteredRows = dataRows
    .filter((r) => {
      if (filterMode === 'known') return !!r.name;
      if (filterMode === 'unknown') return !r.name;
      if (filterMode === 'late') return r.hasLateCall;
      return true;
    })
    .slice()
    .sort((a, b) => {
      if (sortKey === 'rank') {
        // Default: known-first, then call_count desc
        const diff = b.call_count - a.call_count;
      ...
      if (sortKey === 'calls') {
        const diff = a.call_count - b.call_count;
        return sortDir === 'asc' ? diff : -diff;
      }
      if (sortKey === 'duration') {
        const cmp = a.total_duration.localeCompare(b.total_duration);
        return sortDir === 'asc' ? cmp : -cmp;
      }
      return 0;
    });

  const maxCalls = dataRows.length > 0 ? Math.max(...dataRows.map((r) => r.call_count)) : 1;
    ...
    <div className="space-y-5">
      ...
      {hasResults ? (
        <div
          ...
          className={`cursor-pointer flex items-center justify-between gap-4 rounded-2xl border px-5 py-3 transition-colors ${
            dragging
        ...
        >
          ...
          <span className="text-xs text-indigo-600 font-medium border border-indigo-200 rounded-xl px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 transition-colors whitespace-nowrap">
            Change file
          </span>
        </div>
      ) : (
        ...
        >
          ...
          <p className="text-slate-700 font-semibold text-lg">Drag & drop your calls file here</p>
      ...
      {hasResults && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            ...
            <div className="col-span-2 sm:col-span-1 bg-white rounded-2xl border border-slate-200 px-4 py-3 shadow-sm">
            ...
            <div className="px-5 py-3 border-b border-slate-100 flex flex-wrap items-center gap-3 justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">
                  {filteredRows.length} / {dataRows.length} numbers
                </span>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {(
                  [
                    { key: 'all', label: 'All' },
                    { key: 'known', label: `Known (${knownCount})` },
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold transition-colors ml-2"
                >
                  ↓ Download .xlsx
                </button>
              </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 text-slate-500 text-xs uppercase tracking-wide border-b border-slate-100">
                    ...
                    <th
                      className="px-4 py-3 text-right font-semibold cursor-pointer hover:text-indigo-600 select-none transition-colors"
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredRows.map((row, i) => {
                    ...
                    const originalRank = dataRows.indexOf(row);
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5 text-slate-800">
                          <div className="flex items-center gap-2">
                            {row.hasLateCall && (
                              <span title="Has calls after 21:00" className="text-amber-500 text-xs leading-none">⚠</span>
                            )}
                            {row.name ? (
                              ...
                              <div className="flex items-center gap-2">
                                <input
                                  ...
                                  className="px-2 py-1 text-sm rounded-xl border border-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-400 w-36"
                                />
                                <button
                                  onClick={(e) => { e.stopPropagation(); handleSaveName(row.phone); }}
                                  className="text-xs px-2.5 py-1 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-medium transition-colors"
                                >
                                  Save
                                </button>
                                <button
                  ...
                  {/* TOTAL row — only visible in 'all' mode */}
                  {filterMode === 'all' && totalRow && (
                    <tr className="bg-indigo-50 border-t-2 border-indigo-100">
                      <td className="px-4 py-3" />
                      <td className="px-4 py-3 text-xs font-bold text-indigo-700 uppercase tracking-widest">Total</td>
                      <td className="px-4 py-3" />
                      <td className="px-4 py-3 text-right">
                        <span className="font-bold text-indigo-700 text-base tabular-nums">{totalRow.call_count}</span>
                      </td>
                      <td className="px-4 py-3" />
                    </tr>
                  )}
...
function StatCard({ label, value, color = 'slate' }: StatCardProps) {
  const valueColor =
    color === 'emerald' ? 'text-emerald-600' : color === 'amber' ? 'text-amber-600' : 'text-slate-800';
  return (
...
function CallDetailModal({ row, onClose }: CallDetailModalProps) {
  const displayPhone = row.phone.startsWith('0') ? row.phone : '0' + row.phone;
  const lateCallCount = row.calls.filter((c) => c.isLate).length;
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-start justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-3 mt-2 text-xs text-slate-500">
              <span className="bg-slate-100 rounded-full px-2 py-0.5 font-medium">{row.call_count} calls</span>
              <span className="bg-slate-100 rounded-full px-2 py-0.5 font-medium">{row.total_duration} total</span>
              {lateCallCount > 0 && (
                <span className="bg-amber-50 border border-amber-200 text-amber-700 rounded-full px-2 py-0.5 font-medium">
                  ⚠ {lateCallCount} after 21:00
                </span>
              )}
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {row.calls.map((call, i) => {
                  ...
                  return (
                    <tr key={i} className={call.isLate ? 'bg-red-50' : 'hover:bg-slate-50/60'}>
                      <td className={`px-5 py-2.5 text-xs ${call.isLate ? 'text-red-400' : 'text-slate-400'}`}>{i + 1}</td>
                      <td className={`px-5 py-2.5 font-medium ${call.isLate ? 'text-red-700' : 'text-slate-700'}`}>{date}</td>
                      <td className={`px-5 py-2.5 font-mono ${call.isLate ? 'text-red-600 font-semibold' : 'text-slate-600'}`}>
                        {time}
                        {call.isLate && (
                          <span className="ml-1.5 text-xs bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full font-sans font-medium">late</span>
                        )}
                      </td>
